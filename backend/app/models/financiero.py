...
from sqlalchemy import String, ForeignKey, DateTime, Date, Numeric, Enum, Boolean, Text
...
class CuotaAnio(Base):
    ...
    anio: Mapped[int]
    agrupacion_id: Mapped[int] = mapped_column(ForeignKey("agrupacion_territorial.id"))
...
class Donacion(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    miembro_id: Mapped[int] = mapped_column(ForeignKey("miembro.id"))
    concepto_id: Mapped[int | None] = mapped_column(ForeignKey("donacion_concepto.id"))
...
class OrdenCobro(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    remesa_id: Mapped[int] = mapped_column(ForeignKey("remesa.id"))
    cuota_id: Mapped[int] = mapped_column(ForeignKey("cuota_anio.id"))
    importe: Mapped[Decimal] = mapped_column(Numeric(10, 2))
