...
class ImportadorFinancieroComplementario:
    ...
    # Mapeo de conceptos MySQL → código de campaña en PostgreSQL
    MAPEO_CONCEPTO_CAMPANIA = {
    ...
    def __init__(self):
        ...
        self.cache_campanias: dict[str, str] = {}  # codigo_campania → UUID
    ...
    async def cargar_caches(self, session: AsyncSession):
        ...
        # Cargar campañas para asociar donaciones
        result = await session.execute(
            text("SELECT codigo, id FROM campanias")
        )
        for row in result:
            self.cache_campanias[str(row[0])] = str(row[1])
        ...
        print(f"    Campañas: {len(self.cache_campanias)}")
            ...
            async with mysql_conn.cursor() as cursor:
                ...
                while True:
                    ...
                    for row in rows:
                        ...
                        # Buscar campaña asociada al concepto
                        campania_id = None
                        if concepto_cod and concepto_cod in self.MAPEO_CONCEPTO_CAMPANIA:
                            codigo_campania = self.MAPEO_CONCEPTO_CAMPANIA[concepto_cod]
                            campania_id = self.cache_campanias.get(codigo_campania)
                            if campania_id:
                                self.stats['donaciones_con_campania'] += 1
                            ...
                            await session.execute(
                                text("""
                                    INSERT INTO donaciones (
                                        id, concepto_id, campania_id, estado_id, fecha, importe, gastos,
...
async def main():
    ...
    async with async_session() as session:
        try:
            ...
            print(f"  Donaciones: {importador.stats['donaciones']} (omitidas: {importador.stats['donaciones_omitidas']}, con campaña: {importador.stats['donaciones_con_campania']})")
